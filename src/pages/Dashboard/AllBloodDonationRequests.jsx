...
const AllBloodDonationRequests = () => {
  ...
  const fetchData = () => {
    setLoading(true);
    axiosSecure
      .get('/donation-requests/all', { params: { page, limit, status } })
      .then((res) => {
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 className="font-heading text-2xl font-semibold text-gray-900">All Donation Requests</h1>
        <select
          value={status}
          onChange={(e) => {
      ...
      {loading ? (
        ...
        <>
          <DonationRequestsTable
            requests={requests}
            onChanged={fetchData}
            mode={user?.role === 'volunteer' ? 'volunteer' : 'admin'}
          />
